...
def clean_paper_type(value):
    ...
    # Paper Type should be everything after Paper Type and before Bindery.
    value = re.split(
        r"\bBINDERY\b\s*\d*\s*: ?",
        value,
        flags=re.I,
...
def extract_specs(spec_text, desc_fallback="", supplier_part=""):
    ...
    # Paper Type should pull only what is between PAPER TYPE and BINDERY.
    paper = extract_between(src, "PAPER TYPE", ["BINDERY", "BINDERY 1", "BINDERY 2"])

    if not paper and desc:
        paper = extract_between(desc, "PAPER TYPE", ["BINDERY", "BINDERY 1", "BINDERY 2"])
    ...
    # Page Size should pull the full dimensions, such as 25.25 X 14, before COLOR.
    page = extract_between(src, "PAGE SIZE", ["COLOR", "COATING", "PAGE SETUP", "PAPER TYPE", "BINDERY"])

    if not page and desc:
        page = extract_between(desc, "PAGE SIZE", ["COLOR", "COATING", "PAGE SETUP", "PAPER TYPE", "BINDERY"])
...
def load_tracking(upload):
    ...
    needed = ["Job No", "Order Description", "PO#", "Tracking Number", "Shipped By"]
    ...
    return df[["Job No", "Order Description", "PO#", "Tracking Number", "Shipped By", "Desc"]].copy()
    ...
    for row_num in range(2, ws.max_row + 1):
        if paper_col:
            ...
            if not clean_text(paper_cell.value):
                paper_cell.fill = missing_paper_fill

        if page_col:
            page_cell = ws.cell(row_num, page_col)
            page_value = clean_text(page_cell.value)
            if page_value and not re.search(r"\d+(?:\.\d+)?\s*[xX]\s*\d+(?:\.\d+)?", page_value):
                page_cell.fill = bad_page_fill
...
def write_excel(final_df, missing_emails_df, cutoff_exclusions_df, stats):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        final_df[final_df["LAM"].eq("LAM")].to_excel(writer, sheet_name="LAM", index=False)
        final_df[final_df["UV"].eq("UV")].to_excel(writer, sheet_name="UV", index=False)
        final_df[
            (final_df["LAM"].eq("NONE"))
            & (final_df["UV"].eq("NONE"))
...
if st.button("Generate Report", type="primary"):
    ...
    if not tracking_upload:
        ...
        try:
            ...
            cols[2].metric("After Cutoff", stats["After Cutoff Date Exclusions"])
            ...
            if stats["After Cutoff Date Exclusions"]:
                st.info(
                    f"{stats['After Cutoff Date Exclusions']} open jobs were excluded because their USF Date is after the selected cutoff date."
                )
            ...
            st.download_button(
                ...
                data=report_bytes,
                file_name=f"US_Foods_Due_Date_Report_{selected_due_date.strftime('%m%d%Y')}.xlsx",
